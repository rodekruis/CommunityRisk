...
--DUMMY:
/* Not needed any more now risk score is added to normal indicator table
...
haz_risk as (
	select t0.pcode_level4
		,flood_phys_exp --log(1 + flood_phys_exp) as flood_phys_exp
		,drought_phys_exp --log(1 + drought_phys_exp) as drought_phys_exp
	from "MW_datamodel"."Geo_level4" 	t0
	left join "MW_datamodel"."Indicators_4_TOTAL"	t1 on t0.pcode_level4 = t1.pcode
	)
, 
