...
(function (fng) {
    ...
    (function (services) {
        ...
        function routingService($injector, $locationProvider) {
            ...
            var builtInRoutes = [
                {
                    ...
                    templateUrl: 'modules/forms/views/base-analysis.client.view.html'
                },
            ...
            var lastRoute = null;
            var lastObject = {};
            function _setUpNgRoutes(routes, prefix, templatePath, additional) {
                ...
                angular.forEach(routes, function (routeSpec) {
                    _routeProvider.when(prefix + routeSpec.route, angular.extend(routeSpec.options || { templateUrl: templatePath + routeSpec.templateUrl }, additional));
                });
                ...
                if (config.variantsForDemoWebsite) {
                    angular.forEach(config.variantsForDemoWebsite, function (variant) {
                        angular.forEach(routes, function (routeSpec) {
                            _routeProvider.when(prefix + variant + routeSpec.route, angular.extend(routeSpec.options || { templateUrl: templatePath + routeSpec.templateUrl }, additional));
                        });
                    });
                }
            }
            function _setUpUIRoutes(routes, prefix, templatePath, additional) {
                if (prefix === void 0) { prefix = ''; }
                prefix = prefix || '';
                ...
                angular.forEach(routes, function (routeSpec) {
                    _stateProvider.state(routeSpec.state, angular.extend(routeSpec.options || {
                        ...
                        templateUrl: templatePath + routeSpec.templateUrl
                    }, additional));
                });
            }
                ...
                start: function (options) {
                    ...
                    switch (config.routing) {
                        case 'ngroute':
                            ...
                            _setUpNgRoutes(builtInRoutes, config.prefix, config.templatePath, options.add2fngRoutes);
                            break;
                        case 'uirouter':
                            ...
                            setTimeout(function () {
                                _setUpUIRoutes(builtInRoutes, config.prefix, config.templatePath, options.add2fngRoutes);
                            });
                            break;
                    }
                },
