...
angular.module('dashboards')
	...
	function($translate,$scope,$css,$rootScope, $compile, $q, Authentication, Dashboards, Data, Sources, $window, $stateParams, cfpLoadingBar, _) {
		...
		$scope.load_data = function(pgData) {
            ...
            // 4. Variable-metadata
            var replace_null = function(value) { return (value == null) ? "" : value };
            d.Metadata_full = pgData.usp_data.meta_indicators; //dashboard.sources.Metadata.data;
            d.Metadata = $.grep(d.Metadata_full, function(e){ return (((e.view_code == 'PI' || e.view_code == 'CRA,PI') && $scope.replace_null(e.disaster_type).indexOf($scope.disaster_type) > -1) 
                                                                        || ($scope.replace_null(e.view_code).indexOf('PI') > -1 && $scope.replace_null(e.disaster_type) == '')) 
            ...
            // 6. Disaster-metadata
            d.Disaster_meta_full = pgData.usp_data.meta_disaster; //dashboard.sources.Metadata_disaster_events.data;
            d.Disaster_meta = $.grep(d.Disaster_meta_full, function(e){return e.disaster_type == $scope.disaster_type && e.country_code == $scope.country_code;});
		...
		$scope.generateCharts = function (d){
            ...
            $scope.default_metric = $scope.genLookup_disaster_meta(d,'default_metric')[$scope.disaster_name];
			...
			for (var i=0; i < d.Metadata.length; i++) {
				...
				if (record_temp.group !== 'admin') {
					...
					record.view = 'PI'; //record_temp.view_code; 
			...
			var fill_keyvalues = function() {
				...
				$scope.tables.forEach(function(t) {
					var key = t.name;
					if ($scope.admlevel == zoom_max && $scope.filters.length == 0 && !isNaN(d_prev[t.name])) {
			...
			$scope.createHTML = function(keyvalue) {
				...
				if (cra_features) {while (cra_features.firstChild) { cra_features.removeChild(cra_features.firstChild); };}
				...
				for (var i=0;i<$scope.tables.length;i++) {
                    ...
                    if (record.group !== 'hide' && document.getElementById(record.group)) {
						...
						if ( (  !($scope.predictions == 'no' && record.group == 'predictions') 
                             && !($scope.actuals == 'no' && record.group == 'damage') 
                             && !(($scope.predictions == 'no' || $scope.actuals == 'no') && record.group == 'pred_error')
                             ) 
			...
			$scope.mapchartColors = function() {
                ...
                } else if ($scope.genLookup_meta(d,'view_code')[$scope.metric] == 'CRA,PI') {
			...
			mapChart
				...
				.colorCalculator(function(d){
                    if ($scope.metric.indexOf('damage_class') > -1) {
                        ...
                        if (d.sum==1){return colors[0];} else if (d.sum==2){return colors[1];} else if (d.sum==3){return colors[2];} else if (d.sum==4){return colors[3];} else if (d.sum==5){return colors[4];};
                    } else if ($scope.metric == 'pred_error_damage') {
                        var colors = $scope.quantileColorDomain_PI_error;
                        if (!d.count){return '#cccccc';} else if (d.sum<=-3){return colors[0];} else if (d.sum==-2){return colors[1];} else if (Math.abs(d.sum)<=1){return colors[2];} else if (d.sum==2){return colors[3];} else if (d.sum>=3){return colors[4];};
                    } else {
            ...
            rowChart
				...
				.colorCalculator(function(d){
                    if ($scope.metric.indexOf('damage_class') > -1) {
                        var colors = $scope.quantileColorDomain_PI_std;
                        if (d.value.sum==1){return colors[0];} else if (d.value.sum==2){return colors[1];} else if (d.value.sum==3){return colors[2];} else if (d.value.sum==4){return colors[3];} else if (d.value.sum==5){return colors[4];};
                    } else if ($scope.metric == 'pred_error_damage') {
                        var colors = $scope.quantileColorDomain_PI_error;
                        if (!d.value.count){return '#cccccc';} else if (d.value.sum<=-3){return colors[0];} else if (d.value.sum==-2){return colors[1];} else if (Math.abs(d.value.sum)<=1){return colors[2];} else if (d.value.sum==2){return colors[3];} else if (d.value.sum>=3){return colors[4];};
                    } else {
                ...
                .title(function(d){
					if (!meta_scorevar[$scope.metric]){
						return $scope.replace_null(lookup[d.key]).concat(' - ',meta_label[$scope.metric],': ',currentFormat(d.value.sum), ' ',meta_unit[$scope.metric]);
					} else {
			...
			$scope.map_coloring = function(id) {
				...
				mapChart
					...
					.popup(function(d){
						return lookup[d.key].concat(' - ',meta_label[$scope.metric],': ',currentFormat(d.value.sum), ' ',$scope.replace_null(meta_unit[$scope.metric]));
					})
					;
			...
			function addParameterToURL(view,country,admlevel,metric,parent_codes,disaster_type,disaster_name,chart_show){
				var _url = location.href;
				_url = _url.split('?')[0];
				_url += (_url.split('?')[1] ? '&':'?') + 'country='+country+'&admlevel='+admlevel+'&metric='+metric+'&disaster='+disaster_type+'&event='+disaster_name+'&view='+chart_show;
				return _url;
			}
			$scope.share_URL = function() {
				$scope.shareable_URL = addParameterToURL($scope.view_code,$scope.country_code,$scope.admlevel,$scope.metric,$scope.parent_codes,$scope.disaster_type,$scope.disaster_name,$scope.chart_show);
				$('#URLModal').modal('show');
			}
            ...
            for (var i=0;i<d.Disaster_meta_full.length;i++) {
                ...
                if (d.Disaster_meta_full[i].country_code !== null && countries.indexOf(d.Disaster_meta_full[i].country_code) <= -1
                    && (($scope.view_code_PI == 'DDB' && d.Disaster_meta_full[i].actuals == 'yes') ||
                        ($scope.view_code_PI == 'PI' && d.Disaster_meta_full[i].predictions == 'yes'))
                    ) {
                    ...
                    a.innerHTML = $scope.genLookup_country_meta(d,'country_name')[record.country_code];
                    $compile(a)($scope);
                    li.appendChild(a);
            ...
            for (var i=0;i<d.Disaster_meta_full.length;i++) {
                ...
                if (d.Disaster_meta_full[i].country_code == $scope.country_code 
                    ...
                    var a = document.createElement('a');
                    a.setAttribute('class','submenu-item');
                    a.setAttribute('ng-click','change_disaster_type(\'' + record.disaster_type + '\')');
                    a.setAttribute('role','button');
